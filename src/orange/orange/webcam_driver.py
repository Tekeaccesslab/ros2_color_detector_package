...
# ROS 2 Python program to publish real-time streaming  video from your built-in webcam
...
from rclpy.qos import QoSReliabilityPolicy  # To change the QoS Reliability Policy 
from rclpy.qos import QoSDurabilityPolicy   # To change the QoS Durability Policy Not in the requiremnt but I just included 
import time

# def load_images(paths: list[str]) -> tuple[list[NDArrayRGB], list[str]]: ...
class ImagePublisher(Node):
  """
  Create an Image Publisher class, which is a subclass of the Node class.
  """
  def __init__(self):
    """
    ...
    # Change the QoS here (Requirement : BEST EFFERT)
    qos_profile = QoSProfile(
            reliability=QoSReliabilityPolicy.BEST_EFFORT,
            durability=QoSDurabilityPolicy.VOLATILE,
            depth=10  # Adjust the queue depth if needed
        )
    self.publisher_ = self.create_publisher(Image, 'webcam/image_raw', qos_profile)
    # self.publisher_ = self.create_publisher(Image, 'webcam/image_raw', 10) # This is to leave everything as default except the depth 
    #  
